...
import { db, versionsTable, projectsTable, productsTable, reportsTable, buildingsTable } from "@workspace/db";
...
router.get("/:versionId/reports", requireAuth, async (req, res): Promise<void> => {
  ...
  const access = await verifyVersionAccess(versionId, req.session.tenantId!);
  if (!access) { res.status(404).json({ error: "Version not found" }); return; }
...
router.post("/:versionId/reports/pdf", requireAuth, async (req, res): Promise<void> => {
  ...
  for (const p of products) {
    const total = (p.quantityValue ?? 0) * (p.co2ePerUnitSnapshot ?? 0);
    grandTotal += total;
    doc.fontSize(11).text(`${p.name} (${p.category}): ${p.quantityValue ?? 0} ${p.quantityUnit ?? ""} × ${p.co2ePerUnitSnapshot ?? 0} = ${total.toFixed(2)} kg CO2e`);
  }
  doc.moveDown();
...
router.post("/:versionId/reports/xlsx", requireAuth, async (req, res): Promise<void> => {
  ...
  for (const p of products) {
    const total = (p.quantityValue ?? 0) * (p.co2ePerUnitSnapshot ?? 0);
    ws.addRow([
      p.name, p.category, p.quantityValue, p.quantityUnit,
      p.co2ePerUnitSnapshot, p.moduleA1A3Share, p.moduleA4Share,
      p.moduleA5Share, p.moduleBShare, p.moduleCShare, total.toFixed(2),
    ]);
  }
